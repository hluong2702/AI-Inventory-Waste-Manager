...
|   Waste Records)    |  Order Forecast)    |   Rate Limiting)  |   payOS Webhook)  |
+---------------------+---------------------+-------------------+-------------------+
```
...
Các bằng chứng hiện có được tách rõ khỏi mục tiêu cần đo để tránh diễn giải số liệu giả định thành
kết quả production:
*   **Đã kiểm chứng tự động**: test backend bao phủ các ranh giới tenant, FEFO, subscription/payment,
    invitation outbox, report và alert; frontend có lint, coverage gate, API-contract gate,
    accessibility E2E và performance budget.
*   **Budget frontend hiện tại**: trang public phải tải dưới **3 giây**, tổng tài nguyên truyền dưới
    **1.5 MB**, và mỗi JavaScript asset không vượt **500 KB** trong Playwright production preview.
*   **Mục tiêu chưa có bằng chứng production**: p95 API, throughput 1.000 request/giây, uptime 99,9%,
    giảm thời gian kiểm kho 65% và giảm lãng phí 20-30%. Các số này chỉ được công bố sau load test,
    telemetry production hoặc thử nghiệm người dùng có dữ liệu gốc và phương pháp đo lưu trữ được.

---

## 10. Định hướng phát triển tiếp theo (Future Work)

1.  **Đo NFR trên staging**: bổ sung kịch bản load test tái lập được, lưu p50/p95/p99, throughput,
    error rate và cấu hình hạ tầng đi kèm kết quả.
2.  **Mở rộng E2E có xác thực**: chạy các luồng nhập/xuất FEFO, thanh toán và quản lý nhân sự trên
    dữ liệu cô lập bằng Testcontainers hoặc môi trường staging dùng riêng cho CI.
3.  **Thiết lập SLO production**: nối metric `inventoryai.*`, health check và correlation ID vào hệ
    thống dashboard/alerting; đo uptime trước khi cam kết SLO 99,9%.
