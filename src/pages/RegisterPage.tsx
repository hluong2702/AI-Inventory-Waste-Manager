...
export default function RegisterPage() {
  ...
  async function onSubmit(values: RegisterValues) {
    ...
    try {
      const session = await registerOwner(values)
      setSession(session)
    ...
    <div className="min-h-[100dvh] bg-offwhite p-4 md:p-8 text-ink">
      <div className="mx-auto grid min-h-[calc(100dvh-2rem)] max-w-5xl grid-cols-1 overflow-hidden rounded-[28px] border border-ink/10 bg-white shadow-2xl md:grid-cols-12">
        <section className="md:col-span-5 bg-sage-dark p-8 text-white md:p-10">
          ...
          <p className="mt-3 text-sm leading-6 text-white/70">Backend sẽ tạo store mới, tài khoản Owner, gói Free và đăng nhập ngay.</p>
        </section>
        ...
        <main className="md:col-span-7 flex items-center p-6 md:p-10">
          <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-ink/55">Tên quán, email và mật khẩu được lưu qua API backend.</p>
            </div>
            ...
            <Field icon={<Storefront size={17} />} label="Tên quán" error={errors.storeName?.message}>
              <input {...register('storeName')} className="w-full bg-transparent py-3 text-sm outline-none" />
            </Field>
            ...
            <button disabled={isSubmitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-sage-dark px-4 py-3 text-sm font-bold text-white transition hover:bg-sage disabled:opacity-60">
...
function Field({ label, error, icon, children }: { label: string; error?: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <label className="block">
      ...
      <span className="mt-1.5 flex items-center gap-2 rounded-xl border border-ink/10 bg-offwhite/50 px-3 text-ink/35">
        {icon}
        {children}
      </span>
